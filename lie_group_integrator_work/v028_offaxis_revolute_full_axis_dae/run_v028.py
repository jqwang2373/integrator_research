...
CASES = {
    "offaxis_revolute_smooth": {
        "stribeck_velocity": 0.50,
        "external_torque_body": np.array([0.9, 0.0, -0.6]),
    },
...
METHODS = [
    "abs_revolute_gauss4",
    "abs_revolute_gauss6",
    "abs_revolute_gauss4_pivotvc",
    "abs_revolute_gauss6_pivotvc",
    "abs_revolute_gauss4_pivotva",
    "abs_revolute_gauss6_pivotva",
    "abs_revolute_gauss4_fullva",
    "abs_revolute_gauss6_fullva",
]
T_FINAL = 1.0
REF_H = 0.00625
REFERENCE_METHOD = "abs_revolute_gauss6_fullva"
CSV_COLUMNS = [
    "case",
    "method",
    "h",
...
def make_params(stribeck_velocity: float, external_torque_body: np.ndarray | None = None) -> Params:
    if external_torque_body is None:
        external_torque_body = np.zeros(3)
    return Params(
        mass=4.0,
        ...
        external_torque_body=np.asarray(external_torque_body, dtype=float),
    )
...
def base_method(method: str) -> str:
    ...
    if method.endswith("_pivotvc"):
        return method[: -len("_pivotvc")]
    return method
...
def residual_mode(method: str) -> str:
    if "_fullva" in method:
        return "full_va"
    if "_pivotva" in method:
        return "pivot_va"
    if "_pivotvc" in method:
        return "pivot_vc"
    return "raw"
...
def velocity_constraint_parts_np(v: np.ndarray, p: np.ndarray, w: np.ndarray, params: Params) -> tuple[float, float]:
    R = qp.quat_to_rot(p)
    pivot_velocity = v + R @ np.cross(w, params.s_com_to_pivot)
    axis_rate = R @ np.cross(w, params.hinge_axis_body)
    return float(np.linalg.norm(pivot_velocity)), float(np.linalg.norm(np.array([axis_rate[0], axis_rate[2]])))


def acceleration_constraint_parts_np(
    a: np.ndarray, p: np.ndarray, w: np.ndarray, alpha: np.ndarray, params: Params
) -> tuple[float, float]:
    R = qp.quat_to_rot(p)
    pivot_acceleration = a + R @ (
        np.cross(alpha, params.s_com_to_pivot)
        + np.cross(w, np.cross(w, params.s_com_to_pivot))
    )
    axis_acceleration = R @ (
        np.cross(alpha, params.hinge_axis_body)
        + np.cross(w, np.cross(w, params.hinge_axis_body))
    )
    return (
        float(np.linalg.norm(pivot_acceleration)),
        float(np.linalg.norm(np.array([axis_acceleration[0], axis_acceleration[2]]))),
    )
...
def scalar_acceleration_guess(q: float, qdot: float, params: Params) -> float:
    ...
    for _ in range(8):
        ...
        next_alpha = (-mass * 9.81 * length * np.sin(q) + tau + params.external_torque_body[1]) / inertia
...
def residual_core(
    ...
    for i, st in enumerate(stages):
        ...
        axis_acceleration = R_i @ (
            jnp.cross(st["alpha"], hinge_axis_body)
            + jnp.cross(st["w"], jnp.cross(st["w"], hinge_axis_body))
        )
        pivot_velocity = st["v"] + R_i @ jnp.cross(st["w"], s)
        pivot_acceleration = st["a"] + R_i @ (
            jnp.cross(st["alpha"], s) + jnp.cross(st["w"], jnp.cross(st["w"], s))
        )
        u_collocation = st["u"] - h * sum(A[i, j] * ks[j] for j in range(n_stages))
        w_collocation = st["w"] - w0 - h * sum(A[i, j] * stages[j]["alpha"] for j in range(n_stages))
        r_or_pivot_velocity = (
            pivot_velocity
            if mode in ("pivot_vc", "pivot_va", "full_va")
            else st["r"] - r0 - h * sum(A[i, j] * stages[j]["v"] for j in range(n_stages))
        )
        u_or_axis_velocity = (
            jnp.concatenate((u_collocation[1:2], jnp.array([axis_rate[0], axis_rate[2]], dtype=x.dtype)))
            if mode == "full_va"
            else u_collocation
        )
        v_or_pivot_acceleration = (
            pivot_acceleration
            if mode in ("pivot_va", "full_va")
            else st["v"] - v0 - h * sum(A[i, j] * stages[j]["a"] for j in range(n_stages))
        )
        w_or_axis_acceleration = (
            jnp.concatenate(
                (w_collocation[1:2], jnp.array([axis_acceleration[0], axis_acceleration[2]], dtype=x.dtype))
            )
            if mode == "full_va"
            else w_collocation
        )
...
def residual2_pivotvc_core(x, *args):
    return residual_core(x, *args, mode="pivot_vc", n_stages=2)


def residual3_pivotvc_core(x, *args):
    return residual_core(x, *args, mode="pivot_vc", n_stages=3)


def residual2_pivotva_core(x, *args):
    return residual_core(x, *args, mode="pivot_va", n_stages=2)


def residual3_pivotva_core(x, *args):
    return residual_core(x, *args, mode="pivot_va", n_stages=3)


def residual2_fullva_core(x, *args):
    return residual_core(x, *args, mode="full_va", n_stages=2)


def residual3_fullva_core(x, *args):
...
residual2_pivotvc_value = jax.jit(residual2_pivotvc_core)
residual2_pivotvc_jacobian = jax.jit(jax.jacfwd(residual2_pivotvc_core, argnums=0))
residual3_pivotvc_value = jax.jit(residual3_pivotvc_core)
residual3_pivotvc_jacobian = jax.jit(jax.jacfwd(residual3_pivotvc_core, argnums=0))
residual2_pivotva_value = jax.jit(residual2_pivotva_core)
residual2_pivotva_jacobian = jax.jit(jax.jacfwd(residual2_pivotva_core, argnums=0))
residual3_pivotva_value = jax.jit(residual3_pivotva_core)
residual3_pivotva_jacobian = jax.jit(jax.jacfwd(residual3_pivotva_core, argnums=0))
residual2_fullva_value = jax.jit(residual2_fullva_core)
residual2_fullva_jacobian = jax.jit(jax.jacfwd(residual2_fullva_core, argnums=0))
residual3_fullva_value = jax.jit(residual3_fullva_core)
residual3_fullva_jacobian = jax.jit(jax.jacfwd(residual3_fullva_core, argnums=0))


def gauss_step(state: State, h: float, params: Params, n_stages: int, mode: str = "raw") -> tuple[State, int, dict]:
    ...
    elif n_stages == 2 and mode == "pivot_vc":
        value, jacobian, max_iters = residual2_pivotvc_value, residual2_pivotvc_jacobian, 32
    elif n_stages == 3 and mode == "pivot_vc":
        value, jacobian, max_iters = residual3_pivotvc_value, residual3_pivotvc_jacobian, 44
    elif n_stages == 2 and mode == "pivot_va":
        value, jacobian, max_iters = residual2_pivotva_value, residual2_pivotva_jacobian, 36
    elif n_stages == 3 and mode == "pivot_va":
        value, jacobian, max_iters = residual3_pivotva_value, residual3_pivotva_jacobian, 48
    elif n_stages == 2 and mode == "full_va":
        value, jacobian, max_iters = residual2_fullva_value, residual2_fullva_jacobian, 44
    elif n_stages == 3 and mode == "full_va":
        value, jacobian, max_iters = residual3_fullva_value, residual3_fullva_jacobian, 56
    else:
    ...
    max_stage_pivot_velocity_constraint = 0.0
    max_stage_axis_velocity_constraint = 0.0
    max_stage_pivot_acceleration_constraint = 0.0
    max_stage_axis_acceleration_constraint = 0.0
    max_stage_acceleration_constraint = 0.0
    max_lambda = 0.0
    max_power = -np.inf
    min_power = np.inf
    for st in stages:
        ...
        pivot_v, axis_v = velocity_constraint_parts_np(st["v"], p_i, st["w"], params)
        max_stage_pivot_velocity_constraint = max(max_stage_pivot_velocity_constraint, pivot_v)
        max_stage_axis_velocity_constraint = max(max_stage_axis_velocity_constraint, axis_v)
        pivot_a, axis_a = acceleration_constraint_parts_np(st["a"], p_i, st["w"], st["alpha"], params)
        max_stage_pivot_acceleration_constraint = max(max_stage_pivot_acceleration_constraint, pivot_a)
        max_stage_axis_acceleration_constraint = max(max_stage_axis_acceleration_constraint, axis_a)
        max_stage_acceleration_constraint = max(max_stage_acceleration_constraint, float(np.hypot(pivot_a, axis_a)))
        lam_force = st["lambda"][:3]
    ...
    diag = {
        ...
        "max_stage_pivot_velocity_constraint_norm": max_stage_pivot_velocity_constraint,
        "max_stage_axis_velocity_constraint_norm": max_stage_axis_velocity_constraint,
        "max_stage_pivot_acceleration_constraint_norm": max_stage_pivot_acceleration_constraint,
        "max_stage_axis_acceleration_constraint_norm": max_stage_axis_acceleration_constraint,
        "max_stage_acceleration_constraint_norm": max_stage_acceleration_constraint,
        ...
        "max_endpoint_pivot_velocity_constraint_norm": velocity_constraint_parts_np(
            next_state.v, next_state.p, next_state.w, params
        )[0],
        "max_endpoint_axis_velocity_constraint_norm": velocity_constraint_parts_np(
            next_state.v, next_state.p, next_state.w, params
        )[1],
...
def integrate(method: str, h: float, t_final: float, params: Params) -> dict:
    ...
    max_endpoint_pivot_velocity_constraint = 0.0
    max_endpoint_axis_velocity_constraint = 0.0
    ...
    for _ in range(n_steps):
        state, niters, diag = gauss_step(state, h, params, n_stages, mode=mode)
        total_iters += niters
        ...
        endpoint_pivot_velocity, endpoint_axis_velocity = velocity_constraint_parts_np(
            state.v, state.p, state.w, params
        )
        ...
        max_endpoint_pivot_velocity_constraint = max(max_endpoint_pivot_velocity_constraint, endpoint_pivot_velocity)
        max_endpoint_axis_velocity_constraint = max(max_endpoint_axis_velocity_constraint, endpoint_axis_velocity)
        ...
        max_stage_pivot_velocity_constraint = max(
            max_stage_pivot_velocity_constraint, diag["max_stage_pivot_velocity_constraint_norm"]
        )
        max_stage_axis_velocity_constraint = max(
            max_stage_axis_velocity_constraint, diag["max_stage_axis_velocity_constraint_norm"]
        )
        max_stage_pivot_acceleration_constraint = max(
            max_stage_pivot_acceleration_constraint, diag["max_stage_pivot_acceleration_constraint_norm"]
        )
        max_stage_axis_acceleration_constraint = max(
            max_stage_axis_acceleration_constraint, diag["max_stage_axis_acceleration_constraint_norm"]
        )
        max_stage_acceleration_constraint = max(
            max_stage_acceleration_constraint, diag["max_stage_acceleration_constraint_norm"]
        )
    ...
    return {
        ...
        "max_endpoint_pivot_velocity_constraint_norm": max_endpoint_pivot_velocity_constraint,
        "max_endpoint_axis_velocity_constraint_norm": max_endpoint_axis_velocity_constraint,
...
def run_experiment() -> dict:
    ...
    for case_name, case_spec in CASES.items():
        vs = float(case_spec["stribeck_velocity"])
        external_torque_body = np.asarray(case_spec["external_torque_body"], dtype=float)
        params = make_params(vs, external_torque_body)
        ...
        gauss_step(state0, 0.025, params, 2, mode="pivot_vc")
        gauss_step(state0, 0.025, params, 3, mode="pivot_vc")
        gauss_step(state0, 0.025, params, 2, mode="pivot_va")
        gauss_step(state0, 0.025, params, 3, mode="pivot_va")
        gauss_step(state0, 0.025, params, 2, mode="full_va")
        gauss_step(state0, 0.025, params, 3, mode="full_va")
        method_summary = {}
        ...
        cases[case_name] = {
            ...
            "external_torque_body": external_torque_body.tolist(),
            "reference_method": REFERENCE_METHOD,
            "reference_h": REF_H,
            "methods": method_summary,
            "reference": {key: value for key, value in ref.items() if key != "state"},
        }
    write_csv(RESULTS / "offaxis_revolute_full_axis_runs.csv", rows)
...
def plot_results(summary: dict) -> None:
    ...
    plt.title("Off-Axis Revolute Full-Axis DAE Convergence")
    plt.grid(True, which="both", alpha=0.35)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "offaxis_revolute_full_axis_convergence.png", dpi=180)
    plt.close()
    ...
    plt.title("Endpoint Velocity Constraint")
    plt.grid(True, which="both", alpha=0.35)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "offaxis_revolute_full_axis_velocity_constraints.png", dpi=180)
    plt.close()
    ...
    for case_name, case in summary["cases"].items():
        for method in METHODS:
            runs = case["methods"][method]["runs"]
            ok = [
                (float(h), runs[h]["max_stage_axis_acceleration_constraint_norm"])
                for h in runs.keys()
    ...
    plt.gca().invert_xaxis()
    plt.xlabel("step size h")
    plt.ylabel("stage axis acceleration constraint norm")
    plt.title("Off-Axis Stage Axis-Acceleration Constraint")
    plt.grid(True, which="both", alpha=0.35)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "offaxis_revolute_full_axis_acceleration_constraints.png", dpi=180)
    plt.close()
...
def write_report(summary: dict) -> None:
    ...
    for case_name, case in summary["cases"].items():
        ...
        for method in METHODS:
            ...
            if ok_runs:
                ...
                lines.append(
                    ...
                    f"pivot velocity {fine['max_endpoint_pivot_velocity_constraint_norm']:.3e}, "
                    f"axis velocity {fine['max_endpoint_axis_velocity_constraint_norm']:.3e}, "
                    f"stage pivot acceleration {fine.get('max_stage_pivot_acceleration_constraint_norm', float('nan')):.3e}, "
                    f"stage axis acceleration {fine.get('max_stage_axis_acceleration_constraint_norm', float('nan')):.3e}, "
        ...
        pivotvc = case["methods"]["abs_revolute_gauss6_pivotvc"]["runs"].get("0.0125")
        pivotva = case["methods"]["abs_revolute_gauss6_pivotva"]["runs"].get("0.0125")
        fullva = case["methods"]["abs_revolute_gauss6_fullva"]["runs"].get("0.0125")
        if raw and pivotvc and raw.get("status") == "ok" and pivotvc.get("status") == "ok":
            lines.append(
                f"- PivotVC vs raw Gauss6 at h=0.0125: endpoint velocity constraint "
                f"{pivotvc['max_endpoint_velocity_constraint_norm']:.3e} vs {raw['max_endpoint_velocity_constraint_norm']:.3e}; "
                f"orientation error ratio {pivotvc['orientation_error_rad'] / max(raw['orientation_error_rad'], 1e-30):.2e}."
            )
        else:
            lines.append("- PivotVC vs raw Gauss6 at h=0.0125 is not comparable because at least one run failed.")
        if raw and pivotva and raw.get("status") == "ok" and pivotva.get("status") == "ok":
            lines.append(
                f"- PivotVA vs raw Gauss6 at h=0.0125: endpoint velocity constraint "
                f"{pivotva['max_endpoint_velocity_constraint_norm']:.3e} vs {raw['max_endpoint_velocity_constraint_norm']:.3e}; "
                f"stage pivot acceleration {pivotva['max_stage_pivot_acceleration_constraint_norm']:.3e}; "
                f"orientation error ratio {pivotva['orientation_error_rad'] / max(raw['orientation_error_rad'], 1e-30):.2e}."
            )
        else:
            lines.append("- PivotVA vs raw Gauss6 at h=0.0125 is not comparable because at least one run failed.")
        if pivotvc and pivotva and pivotvc.get("status") == "ok" and pivotva.get("status") == "ok":
            lines.append(
                f"- PivotVA vs PivotVC Gauss6 at h=0.0125: endpoint velocity constraint "
                f"{pivotva['max_endpoint_velocity_constraint_norm']:.3e} vs {pivotvc['max_endpoint_velocity_constraint_norm']:.3e}; "
                f"orientation error ratio {pivotva['orientation_error_rad'] / max(pivotvc['orientation_error_rad'], 1e-30):.2e}."
            )
        else:
            lines.append("- PivotVA vs PivotVC Gauss6 at h=0.0125 is not comparable because at least one run failed.")
        if pivotva and fullva and pivotva.get("status") == "ok" and fullva.get("status") == "ok":
            lines.append(
                f"- FullVA vs PivotVA Gauss6 at h=0.0125: stage axis acceleration "
                f"{fullva['max_stage_axis_acceleration_constraint_norm']:.3e} vs "
                f"{pivotva['max_stage_axis_acceleration_constraint_norm']:.3e}; "
                f"endpoint axis velocity {fullva['max_endpoint_axis_velocity_constraint_norm']:.3e} vs "
                f"{pivotva['max_endpoint_axis_velocity_constraint_norm']:.3e}; "
                f"orientation error ratio {fullva['orientation_error_rad'] / max(pivotva['orientation_error_rad'], 1e-30):.2e}."
            )
        else:
            lines.append("- FullVA vs PivotVA Gauss6 at h=0.0125 is not comparable because at least one run failed.")
        raw4_025 = case["methods"]["abs_revolute_gauss4"]["runs"].get("0.025")
        vc4_025 = case["methods"]["abs_revolute_gauss4_pivotvc"]["runs"].get("0.025")
        va4_025 = case["methods"]["abs_revolute_gauss4_pivotva"]["runs"].get("0.025")
        full4_025 = case["methods"]["abs_revolute_gauss4_fullva"]["runs"].get("0.025")
        raw4_0125 = case["methods"]["abs_revolute_gauss4"]["runs"].get("0.0125")
        vc4_0125 = case["methods"]["abs_revolute_gauss4_pivotvc"]["runs"].get("0.0125")
        va4_0125 = case["methods"]["abs_revolute_gauss4_pivotva"]["runs"].get("0.0125")
        full4_0125 = case["methods"]["abs_revolute_gauss4_fullva"]["runs"].get("0.0125")
        if (
            raw4_025
            and vc4_025
            and raw4_0125
            ...
            and vc4_025.get("status") == "ok"
            and vc4_0125.get("status") == "ok"
        ):
            lines.append(
                "- PivotVC rescued Gauss4 convergence at h=0.025 and h=0.0125, "
                f"with PivotVC h=0.0125 orientation error {vc4_0125['orientation_error_rad']:.3e}."
            )
        if (
            raw4_025
            and va4_025
            ...
            and raw4_025.get("status") == "failed"
            and raw4_0125.get("status") == "failed"
            and va4_025.get("status") == "ok"
            and va4_0125.get("status") == "ok"
        ):
            lines.append(
                "- PivotVA rescued Gauss4 convergence at h=0.025 and h=0.0125, "
                f"with PivotVA h=0.0125 orientation error {va4_0125['orientation_error_rad']:.3e}."
            )
        if (
            raw4_025
            and full4_025
            ...
            and raw4_025.get("status") == "failed"
            and raw4_0125.get("status") == "failed"
            and full4_025.get("status") == "ok"
            and full4_0125.get("status") == "ok"
        ):
            lines.append(
                "- FullVA rescued Gauss4 convergence at h=0.025 and h=0.0125, "
                f"with FullVA h=0.0125 orientation error {full4_0125['orientation_error_rad']:.3e}."
            )
        lines.append("")
    lines.extend(
        [
    ...
    (RESULTS / "v028_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    summary = {
        "version": "v028_offaxis_revolute_full_axis_dae",
        ...
        "model": {
            ...
            "reference_h": REF_H,
            "t_final": T_FINAL,
            "constraints": "pivot position plus hinge-axis alignment; FullVA adds stage pivot and hinge-axis velocity/acceleration consistency",
            "endpoint_projection": "not used in v028 method set",
            "new_feature": "off-axis torque stress test plus square full-axis velocity/acceleration residual",
        },
        "experiment": experiment,
        "runtime_sec": time.perf_counter() - started,
    }
    with (RESULTS / "summary_v028.json").open("w", encoding="utf-8") as f:
