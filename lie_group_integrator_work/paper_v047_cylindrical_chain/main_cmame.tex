...
\paperfig{\figpath/method_stage_architecture.png}
...
argument is not sharp. On the benchmark, Table~\ref{tab:p2-constants-check}
gives $h_0\approx2\times10^{-5}$ in the Euclidean norm on the implemented
stage layout and $h_0\approx8\times10^{-4}$ in the endpoint-linearized
residual norm $\|J_0^{-1}\,\cdot\,\|$, while the conclusion
$\|J_h^{-1}\|\le2\|J_0^{-1}\|$ is observed with ratio at most $1.55$ on
every reported grid. The gap is the curvature of the Brown--McPhee friction
law in the Newton--Euler rows: without joint friction the same mechanism has
$h_0\approx1.4\times10^{-2}$ in the endpoint-linearized norm, and the
Lipschitz constant of the Jacobian is then set by the gyroscopic terms.
...
Table~\ref{tab:predictor-check} shows that the first full Newton step from
...
$10^{-12}$, giving $J_0$. Table~\ref{tab:p2-constants-check} records
$M_0=\max_n\|J_0^{-1}\|$, $C_J=\max_n\|J_h-J_0\|/h$, the observed ratio
$\|J_h^{-1}\|/\|J_0^{-1}\|$, and the threshold $h_0=1/(2M_0C_J)$ implied by
the Neumann argument, in the Euclidean norm on the implemented stage layout
and in the endpoint-linearized residual norm $\|F\|_{x_n}=\|J_0(x_n)^{-1}F\|$,
the length of the simplified Newton correction, in which $\|J_0^{-1}\|=1$ and
the Neumann argument is sharpest. The conclusion of
Lemma~\ref{lem:p2-from-p1} holds on every grid with ratio at most $1.55$,
but its sufficient condition $M_0\|J_h-J_0\|\le\tfrac12$ would require
$h\le2\times10^{-5}$ in the Euclidean norm and $h\le8\times10^{-4}$ in the
endpoint-linearized norm. The largest entries of $(J_h-J_0)/h$ sit in the
Newton--Euler rows against the stage velocities, where the second derivative
of the Brown--McPhee friction law with Stribeck velocity $0.5$ enters; the
same mechanism with $\mu_s=\mu_d=0$ and no viscous damping has
$C_J$ smaller by a factor of about fifteen, $h_0\approx1.4\times10^{-2}$ in
the endpoint-linearized norm, and its largest Jacobian variation in the
gyroscopic rows. The friction law, not the collocation structure, sets the
small-step threshold, which is consistent with the coarse-grid order
reduction of the sharp case with Stribeck velocity $0.05$.
Table~\ref{tab:predictor-check} records the distances to $Z_G$ of the lifted
endpoint predictor and of the Algorithm~1 predictor: the former decreases
with $h$ as Lemma~\ref{lem:newton-envelope} predicts, whereas the latter
stays at distance about $50$ because its angular-velocity and
angular-acceleration guesses are zero, and the first full Newton step from it
increases the distance by up to $18\%$ before the iteration converges. These
are diagnostics of the lemma constants, not proof inputs.

\begin{table}[htbp]
\centering
\caption{Constants of Lemma~\ref{lem:p2-from-p1} on the smooth cylindrical chain: $M_0=\max_n\|J_0^{-1}\|$, $C_J=\max_n\|J_h-J_0\|/h$, observed inverse ratio $\|J_h^{-1}\|/\|J_0^{-1}\|$, and Neumann threshold $h_0=1/(2M_0C_J)$, in the Euclidean norm on the implemented stage layout (E) and in the endpoint-linearized residual norm $\|J_0^{-1}\,\cdot\,\|$ (L, where $M_0=1$); the last two columns repeat the L-norm quantities for the same mechanism without joint friction.}
\label{tab:p2-constants-check}
\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{P{0.05\linewidth}P{0.07\linewidth}P{0.10\linewidth}P{0.08\linewidth}P{0.10\linewidth}P{0.10\linewidth}P{0.10\linewidth}P{0.10\linewidth}P{0.10\linewidth}}
\toprule
$h$ & $M_0$ (E) & $C_J$ (E) & inverse ratio & $h_0$ (E) & $C_J$ (L) & $h_0$ (L) & $C_J$ (L, no friction) & $h_0$ (L, no friction) \\
\midrule
0.04 & 18.2 & $7.5\times10^{2}$ & 1.55 & $3.7\times10^{-5}$ & $5.1\times10^{2}$ & $9.8\times10^{-4}$ & 34.1 & $1.5\times10^{-2}$ \\
0.02 & 24.3 & $8.2\times10^{2}$ & 1.26 & $2.5\times10^{-5}$ & $5.6\times10^{2}$ & $8.9\times10^{-4}$ & 35.5 & $1.4\times10^{-2}$ \\
0.01 & 28.4 & $9.6\times10^{2}$ & 1.18 & $1.8\times10^{-5}$ & $6.5\times10^{2}$ & $7.6\times10^{-4}$ & 36.5 & $1.4\times10^{-2}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Predictor distances for Lemma~\ref{lem:newton-envelope} on the smooth cylindrical chain (Euclidean norm on the implemented stage layout): distance of the lifted endpoint predictor $Z^{(0)}_\ast$ and of the Algorithm~1 predictor to $Z_G$, and the ratio $\|Z^{(1)}-Z_G\|/\|Z^{(0)}-Z_G\|$ of the first full Newton step from the Algorithm~1 predictor.}
\label{tab:predictor-check}
\small
\begin{tabular}{P{0.08\linewidth}P{0.20\linewidth}P{0.20\linewidth}P{0.20\linewidth}}
\toprule
$h$ & lifted predictor & Algorithm~1 predictor & first-step ratio \\
\midrule
0.04 & 3.23 & 50.2 & 1.12 \\
0.02 & 2.11 & 50.3 & 1.16 \\
0.01 & 1.07 & 50.4 & 1.18 \\
...
\paperfig{\figpath/order_closure_blend.png}
...
\paperfig{\figpath/velocity_compression.png}
...
\paperfig{\figpath/sparse_speed_gap.png}
...
\paperfig{\figpath/strict_common_reference_work_precision.png}
...
\paperfig{\figpath/claim_boundary_limitations.png}
...
\paperfig{\figpath/coarse_baseline_work_precision.png}
...
\paperfig{\figpath/closed_loop_true_dynamic_order.png}
...
\paperfig{\figpath/all_method_result_matrix.png}
...
\paperfig{\figpath/work_precision_compendium.png}
...
\begin{enumerate}
  ...
  (Table~\ref{tab:predictor-check}), the reported production runs use a
  ...
  threshold $h_0$ is below the reported step sizes in every norm tried,
  because of the friction-law curvature. Discharging P6 would in any case
  address the solver-scale
