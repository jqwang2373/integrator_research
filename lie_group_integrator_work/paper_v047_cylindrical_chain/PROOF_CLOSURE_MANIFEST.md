...
- Local defect/global order target: `7` / `6`.
...
- Newton-Euler closure-contract open rows: `0`.
...
- Newton-Euler D5 P_state PS2 Lie-chart binding recorded/SO3/full-mean/full-PS2/factor/gaps/PC2: `True` / `True` / `False` / `False` / `1.0104931253052678` / `2` / `False`.
...
- Newton-Euler D5 P_state PS2 finite linearization probe recorded/full-rank/rank-range/min-singular/state-projection/uniform/PC2: `True` / `True` / `[108, 108]` / `0.009999780246803636` / `2.0194700009526936` / `False` / `False`.
...
- Formula-row AD Jacobian probes/max mismatch: `3` / `3.330669e-16`.
- Smooth projected position/velocity orders: `7.160828` / `7.066183`.
- Endpoint raw constraint fits: `5.985101` / `6.072291`.
- Smooth reference eta/h^7 diagnostic: `3584.657363`.
...
- Finite tolerance-regime sweep policies/rows/steps: `3` / `12` / `90`.
...
- Primitive/Taylor open primitive count: `5`.
...
| item | main | flat |
|---|---:|---:|
| `regularity_assumption` | `True` | `True` |
| `endpoint_closure_lemma` | `True` | `True` |
| `fullva_stage_lift_lemma` | `True` | `True` |
| `newton_euler_obligation_table` | `True` | `True` |
| `newton_euler_row_target_map` | `True` | `True` |
| `stage_residual_defect_lemma` | `True` | `True` |
| `reference_order_contract_lemma` | `True` | `True` |
| `p6_p7_nonclosing_lemma` | `True` | `True` |
| `same_object_composition_lemma` | `True` | `True` |
| `same_object_taylor_transport_gate_lemma` | `True` | `True` |
| `full_132_row_residual_bridge_lemma` | `True` | `True` |
| `inexact_newton_lemma` | `True` | `True` |
| `p6_reported_log_nonclosure_lemma` | `True` | `True` |
| `conditional_order_theorem` | `True` | `True` |
| `order_comparison_proposition` | `True` | `True` |
| `proof_traceability_table` | `True` | `True` |
| `no_reverse_taylor_inference_lemma` | `True` | `True` |
| `p6_solver_policy_obligation_table` | `True` | `True` |
| `p1p2_compact_tube_obligation_table` | `True` | `True` |
| `p3p4_implementation_defect_obligation_table` | `True` | `True` |
| `p5_direct_route_satisfaction_table` | `True` | `True` |
| `p7_residual_to_error_obligation_table` | `True` | `True` |
| `dynamic_proof_closure_matrix` | `True` | `True` |
| `d5_same_object_lift_instantiation_lemma` | `True` | `True` |

## Theorem Assumption Manuscript Anchors

| id | label keys | main lines | flat lines |
|---|---|---|---|
| `P1` | `regularity_assumption,p1p2_compact_tube_obligation_table` | `regularity_assumption:2377,p1p2_compact_tube_obligation_table:3488` | `regularity_assumption:2377,p1p2_compact_tube_obligation_table:3488` |
| `P2` | `regularity_assumption,endpoint_closure_lemma,inexact_newton_lemma,conditional_order_theorem,p1p2_compact_tube_obligation_table` | `regularity_assumption:2377,endpoint_closure_lemma:3828,inexact_newton_lemma:7396,conditional_order_theorem:9118,p1p2_compact_tube_obligation_table:3488` | `regularity_assumption:2377,endpoint_closure_lemma:3828,inexact_newton_lemma:7396,conditional_order_theorem:9118,p1p2_compact_tube_obligation_table:3488` |
| `P3` | `dynamic_proof_closure_matrix,p3p4_implementation_defect_obligation_table` | `dynamic_proof_closure_matrix:4387,p3p4_implementation_defect_obligation_table:3648` | `dynamic_proof_closure_matrix:4387,p3p4_implementation_defect_obligation_table:3648` |
| `P4` | `fullva_stage_lift_lemma,stage_residual_defect_lemma,full_132_row_residual_bridge_lemma,p3p4_implementation_defect_obligation_table` | `fullva_stage_lift_lemma:4058,stage_residual_defect_lemma:6962,full_132_row_residual_bridge_lemma:7217,p3p4_implementation_defect_obligation_table:3648` | `fullva_stage_lift_lemma:4058,stage_residual_defect_lemma:6962,full_132_row_residual_bridge_lemma:7217,p3p4_implementation_defect_obligation_table:3648` |
| `P5` | `newton_euler_obligation_table,newton_euler_row_target_map,stage_residual_defect_lemma,d5_same_object_lift_instantiation_lemma,full_132_row_residual_bridge_lemma,p5_direct_route_satisfaction_table` | `newton_euler_obligation_table:4192,newton_euler_row_target_map:4236,stage_residual_defect_lemma:6962,d5_same_object_lift_instantiation_lemma:4467,full_132_row_residual_bridge_lemma:7217,p5_direct_route_satisfaction_table:3729` | `newton_euler_obligation_table:4192,newton_euler_row_target_map:4236,stage_residual_defect_lemma:6962,d5_same_object_lift_instantiation_lemma:4467,full_132_row_residual_bridge_lemma:7217,p5_direct_route_satisfaction_table:3729` |
| `P6` | `inexact_newton_lemma,p6_reported_log_nonclosure_lemma,conditional_order_theorem,p6_solver_policy_obligation_table` | `inexact_newton_lemma:7396,p6_reported_log_nonclosure_lemma:7558,conditional_order_theorem:9118,p6_solver_policy_obligation_table:3262` | `inexact_newton_lemma:7396,p6_reported_log_nonclosure_lemma:7558,conditional_order_theorem:9118,p6_solver_policy_obligation_table:3262` |
| `P7` | `proof_traceability_table,p7_residual_to_error_obligation_table` | `proof_traceability_table:10732,p7_residual_to_error_obligation_table:8092` | `proof_traceability_table:10732,p7_residual_to_error_obligation_table:8092` |
...
|---|---|---|---:|---:|---:|---|
| `P1` | regular smooth FullVA lift exists on the proof tube | `theorem_condition_retained` | `True` | `False` | `False` | regular_smooth_fullva_lift_required=true |
| `P2` | stage Jacobian is uniformly invertible on the compact-tube Gauss-predictor branch, endpoint closure has a closed anchor, local ball margin, and local right inverse, the accepted one-step map has 1+C_s h stability with tube-retention bootstrap, the local-to-global transfer is a same-initial-state reported-grid estimate on the accepted reduced-chart branch with reported time grid t_n=n h and no exact final-time divisibility requirement, the reported q/v bound uses the same reported time grid as a grid maximum, and the inexact Newton residual estimate has a strong local residual inverse | `theorem_condition_retained` | `True` | `False` | `False` | stage_jacobian_uniformly_invertible_required=true; stage_jacobian_uniform_inverse_required_on_gauss_predictor_branch=true; finite_rank_probes_sufficient_for_stage_inverse=false; accepted_endpoint_closure_closed_anchor_required=true; accepted_endpoint_closure_local_ball_margin_required=true; accepted_endpoint_closure_local_right_inverse_required=true; endpoint_closure_perturbation_constant_formula=C_E = 4 M_E^ri C_{E,raw}; accepted_one_step_stability_required=true; accepted_tube_retention_bootstrap_required=true; local_global_reduced_grid_constant_formula=C_red = C_loc Gamma_s(T); qv_reporting_constant_formula=C_qv = C_{\mathcal R} C_red; local_global_transfer_grid_point_error_required=true; reported_time_grid_defined_by_tn_equals_nh=true; exact_final_time_divisibility_required=false; reported_qv_error_bound_uses_same_reported_time_grid=true; reported_qv_error_bound_is_grid_maximum=true; accepted_newton_residual_strong_local_inverse_required=true; newton_eta_h_scaled_endpoint_bound_formula=C_N c_eta h^7 |
| `P3` | implemented 132-row residual matches the audited row formulas | `runtime_formula_and_ad_oracle_checked_symbolic_oracle_open` | `True` | `False` | `False` | 132 formula rows checked and 3 AD Jacobian probes matched R_JAC |
| `P4` | non-dynamic rows have O(h^7) stage defect on the smooth lift | `binding_interface_retained_96_row_certificate_proved` | `True` | `True` | `False` | 96 kinematic/lower-pair rows certified; 36 Newton-Euler rows excluded |
| `P5` | Newton-Euler rows have O(h^7) implementation defect | `d5_direct_substitution_certificate_closed` | `False` | `True` | `True` | D5_DYNAMIC_DIRECT_SUBSTITUTION_CERTIFICATE proves 36 dynamic rows have zero residual on Z_G using closed D1/D2/D3/D4/D6 inputs and no circular P_state/P_acc/P_lambda route |
| `P6` | inexact Newton residuals obey eta_h^tube <= c_eta h^7 on the compact proof tube under the strong local residual inverse bound; the reported-grid eta_h^reported=max_n eta_{h,n} over accepted branch-selected Newton solves is only the accepted trajectory specialization | `finite_probe_recorded_theorem_condition_retained` | `True` | `False` | `False` | accepted_newton_residual_strong_local_inverse_required=true; same reduced-chart initial state and Gauss-predictor branch selection retained; arbitrary Newton initializations and remote nonlinear roots excluded; summary residuals recorded; finite one-step scaled-tolerance probe has 4/4 rows ok; finite short-trajectory scaled-tolerance probe has 4/4 rows and 30 steps ok; uniform all-transition scaled solver-policy theorem still open and not needed to state the conditional theorem; closure would be needed only to promote P6 from retained hypothesis to discharged solver theorem |
| `P7` | closed-loop residual rates imply trajectory error rates | `open_residual_to_error_transfer` | `False` | `False` | `False` | 7 residual-to-error obligations remain blocking |
...
|---|---:|---|---|
| `gauss_stage_dynamic_defect_rate` | `36` | `symbolic_primitive_route_open` | primitive/Taylor or global symbolic-certificate route for the dynamic-row residual; the active direct D5 substitution route is recorded separately and is closed |

## Newton-Euler Closed Sub-Obligations

| id | rows | status | closure evidence |
|---|---:|---|---|
| `translational_balance_identity` | `18` | `closed` | NEWTON_EULER_BALANCE_IDENTITY_AUDIT.md/json |
| `rotational_balance_identity` | `18` | `closed` | NEWTON_EULER_BALANCE_IDENTITY_AUDIT.md/json |
| `multiplier_wrench_consistency` | `36` | `closed` | NEWTON_EULER_VIRTUAL_WORK_WRENCH_AUDIT.md/json |
| `smooth_force_lift_consistency` | `36` | `closed` | SMOOTH_FORCE_LIFT_CERTIFICATE.md/json |
| `symbolic_runtime_row_equivalence` | `36` | `closed` | NEWTON_EULER_ROW_ORDERING_SCALING_AD_AUDIT.md/json |

## Proof Route Checks

| id | requirement | traceability status | mode |
|---|---|---|---|
| `PC1` | independent symbolic row-by-row oracle for expanded FullVA rows | `traceable: balance-identity route closed` | `D1/D2 balance identities closed by NEWTON_EULER_BALANCE_IDENTITY_AUDIT` |
| `PC2` | O(h^7) implementation residual-defect certificate | `traceable: direct D5 residual-value bridge satisfied` | `D5 same-branch residual-value certificate: 96-row O(h^7) non-dynamic certificate plus 36 dynamic zero rows at Z_G` |
| `PC3` | explicit eta_h^tube <= c_eta h^7 solver-policy theorem or theorem condition retained | `traceable only by retained P6 theorem condition; solver-policy theorem not proved` | `explicit_theorem_condition_retained_not_empirical_solver_evidence` |
| `PC4` | residual-to-error obligations closed before four-link/slider-crank residual promotion | `transfer-scope exclusion retained; residual-to-error theorem not closed` | `nonpromotion_boundary_retained_residual_to_error_theorem_open` |
