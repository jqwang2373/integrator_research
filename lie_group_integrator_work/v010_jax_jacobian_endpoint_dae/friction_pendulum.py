...
def _exp_so3_jax(w):
    ...
    small = eye + W + 0.5 * W2 + (W2 @ W) / 6.0
    theta_safe = jnp.where(theta < 1.0e-8, 1.0, theta)
    large = eye + jnp.sin(theta_safe) / theta_safe * W + (
        (1.0 - jnp.cos(theta_safe)) / (theta_safe * theta_safe)
    ) * W2
    return jnp.where(theta < 1.0e-8, small, large)


def _compose_right_jax(R, u):
    return R @ _exp_so3_jax(u)
...
def _endpoint_residual_jax_core(
    ...
    for i, st in enumerate(stages):
        R_i = _compose_right_jax(R0, st["u"])
    ...
    R_end = _compose_right_jax(R0, end_u)
    out.extend(
        [
...
if JAX_AVAILABLE:
    _endpoint_residual_jax_value = jax.jit(_endpoint_residual_jax_core)
    _endpoint_residual_jax_jacobian = jax.jit(jax.jacfwd(_endpoint_residual_jax_core, argnums=0))
...
def absolute_gauss_endpoint_step(
    ...
    if jacobian_mode not in {"finite_difference", "jax"}:
        raise ValueError(f"unknown jacobian_mode: {jacobian_mode}")
    if jacobian_mode == "jax" and not JAX_AVAILABLE:
        raise RuntimeError("JAX Jacobian requested, but jax is not installed")
    ...
    for it in range(max_iters):
        if jacobian_mode == "jax":
            ...
            res = np.asarray(_endpoint_residual_jax_value(x_jax, *jax_args), dtype=float)
        else:
            res = residual(x)
        last_norm = float(np.linalg.norm(res))
        if last_norm < tol:
            break
        if jacobian_mode == "jax":
            J = np.asarray(_endpoint_residual_jax_jacobian(x_jax, *jax_args), dtype=float)
        else:
            J = finite_difference_jacobian(residual, x)
        delta = np.linalg.solve(J, -res)
        x = x + delta
...
def integrate(
    ...
    for _ in range(n_steps):
        if method == "absolute_gauss_lie4":
            ...
            max_stage_torque = max(max_stage_torque, diag["max_stage_torque_residual_norm"])
            max_lambda = max(max_lambda, diag["lambda_norm_max"])
        elif method == "absolute_gauss_lie4_endpoint_jax":
            state, niters, diag = absolute_gauss_endpoint_step(state, h, params, jacobian_mode="jax")
            total_iters += niters
            max_stage_constraint = max(max_stage_constraint, diag["max_stage_constraint_norm"])
            max_stage_force = max(max_stage_force, diag["max_stage_force_residual_norm"])
            max_stage_torque = max(max_stage_torque, diag["max_stage_torque_residual_norm"])
