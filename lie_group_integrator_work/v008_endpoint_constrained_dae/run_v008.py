...
def run_order() -> dict:
    ...
    t_final = 2.0
    hs = [0.2, 0.1, 0.05]
    methods = ["reduced_rkmk4", "absolute_gauss_lie4_acc_projected", "absolute_gauss_lie4_endpoint"]
    ...
    for method in methods:
        ...
        for h in hs:
            ...
            out = integrate(
                ...
                if method == "absolute_gauss_lie4_acc_projected"
                else method,
                h,
                t_final,
                params,
                project_endpoint=method == "absolute_gauss_lie4_acc_projected",
            )
...
def plot_results(summary: dict) -> None:
    ...
    plt.figure(figsize=(7.0, 4.5))
    for method in ["absolute_gauss_lie4_acc_projected", "absolute_gauss_lie4_endpoint"]:
...
def main() -> None:
    ...
    summary = {
        "version": "v008_endpoint_constrained_dae",
    ...
    endpoint = summary["order"]["methods"]["absolute_gauss_lie4_endpoint"]
    acc_fine = acc["runs"]["0.05"]
    endpoint_fine = endpoint["runs"]["0.05"]
    medium = summary["medium_time"]["methods"]["absolute_gauss_lie4_endpoint"]["0.05"]
    report = [
    ...
    report.extend(
        [
            ...
            f"- Endpoint-constrained `absolute_gauss_lie4_endpoint` at h=0.05: orientation error {endpoint_fine['orientation_error_rad']:.3e}, endpoint constraint {endpoint_fine['max_endpoint_constraint_norm']:.3e}, endpoint velocity constraint {endpoint_fine['max_endpoint_velocity_constraint_norm']:.3e}, max lambda norm {endpoint_fine['max_lambda_norm']:.3e}.",
            f"- Medium-time endpoint-constrained run, 20s h=0.05: energy drift {medium['max_energy_relative_error']:.3e}, endpoint constraint {medium['max_endpoint_constraint_norm']:.3e}, stage constraint {medium['max_stage_constraint_norm']:.3e}, total Newton iterations {medium['total_newton_iterations']}.",
            "",
            "## Interpretation",
            "",
    ...
    (RESULTS / "v008_report.md").write_text("\n".join(report), encoding="utf-8")
