...
def residual_mode(method: str) -> str:
    return "pivot_vc" if "_pivotvc" in method else "raw"
...
def run_experiment() -> dict:
    ...
    write_csv(RESULTS / "absolute_revolute_stage_velocity_runs.csv", rows)
...
def plot_results(summary: dict) -> None:
    ...
    plt.title("Stage Velocity Absolute Revolute DAE Convergence")
    plt.grid(True, which="both", alpha=0.35)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "absolute_revolute_stage_velocity_convergence.png", dpi=180)
    plt.close()
    ...
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "absolute_revolute_stage_velocity_constraints.png", dpi=180)
    plt.close()
...
def write_report(summary: dict) -> None:
    ...
    for case_name, case in summary["cases"].items():
        ...
        proj4_025 = case["methods"]["abs_revolute_gauss4_pivotvc"]["runs"].get("0.025")
        raw4_0125 = case["methods"]["abs_revolute_gauss4"]["runs"].get("0.0125")
        proj4_0125 = case["methods"]["abs_revolute_gauss4_pivotvc"]["runs"].get("0.0125")
        if (
            raw4_025
            and proj4_025
            and raw4_0125
            ...
            lines.append(
                ...
                f"with PivotVC h=0.0125 orientation error {proj4_0125['orientation_error_rad']:.3e}."
            )
        lines.append("")
    lines.extend(
        [
    ...
    (RESULTS / "v026_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    summary = {
        "version": "v026_absolute_revolute_stage_velocity_dae",
        ...
        "model": {
            ...
            "new_feature": "square stage pivot velocity constraint residual",
        },
        "experiment": experiment,
        "runtime_sec": time.perf_counter() - started,
    }
    with (RESULTS / "summary_v026.json").open("w", encoding="utf-8") as f:
