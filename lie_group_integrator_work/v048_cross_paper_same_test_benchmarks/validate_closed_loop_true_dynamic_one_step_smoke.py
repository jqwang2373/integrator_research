#!/usr/bin/env python3
"""Validate the closed-loop true-dynamic one-step smoke artifact."""
...
def main() -> int:
    checks = Checks()
    try:
        rows = read_csv(RESULTS / "closed_loop_true_dynamic_one_step_smoke.csv")
        summary = read_json(RESULTS / "closed_loop_true_dynamic_one_step_smoke.json")
        report = (RESULTS / "closed_loop_true_dynamic_one_step_smoke.md").read_text(encoding="utf-8")
    except Exception as exc:  # noqa: BLE001 - concise CLI failure.
        print(f"closed-loop true dynamic one-step smoke validation: FAIL\n- {exc}")
        return 1

    checks.check(summary.get("schema") == "closed-loop-true-dynamic-one-step-smoke-v1", "schema changed")
    checks.check(summary.get("status") == "one_step_smoke_passed_order_rows_not_run", "status changed")
    ...
    checks.check(summary.get("step_policy") == "oracle_initialized_one_step_smoke_not_order", "step policy changed")
    ...
    checks.check(float(summary.get("max_stage_residual_inf", "nan")) < 1.0e-10, "stage residual too large")
    ...
    checks.check(len(rows) == 2, "expected one one-step row per missing closed-loop model")
    ...
    for row in rows:
        ...
        checks.check(row.get("step_policy") == "oracle_initialized_one_step_smoke_not_order", f"{model} step policy changed")
        ...
        checks.check(as_float(row.get("max_stage_residual_inf", "nan")) < 1.0e-10, f"{model} stage residual too large")
        ...
        checks.check(row.get("trajectory_stepper_implemented") == "true", f"{model} stepper not recorded")
    ...
    for token in [
        ...
        "one-step smoke passed; order rows not run",
    ...
    if checks.errors:
        print("closed-loop true dynamic one-step smoke validation: FAIL")
        for error in checks.errors:
            print(f"- {error}")
        return 1

    print("closed-loop true dynamic one-step smoke validation: PASS")
    print("rows_ok=2/2")
    print(f"max_stage_residual_inf={float(summary['max_stage_residual_inf']):.6e}")
    print(f"max_endpoint_pos_error_inf={float(summary['max_endpoint_pos_error_inf']):.6e}")
