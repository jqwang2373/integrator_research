#!/usr/bin/env python3
"""Run one coarse same-window 2021 public double-pendulum shard."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--step-sizes",
        default=",".join(str(h) for h in rv.RA2021_DOUBLE_COARSE_STEP_SIZES),
    )
    parser.add_argument("--reference-h", type=float, default=rv.RA2021_DOUBLE_COARSE_REFERENCE_H)
    ...
    rv.require_source_policy_1e4_allow(
        ...
        "2021 double coarse shard",
        step_sizes + (float(args.reference_h),),
    )
    config = rv.RA2021DoubleOrderConfig(
        policy="ra2021_double_pendulum_coarse_same_window_policy_not_full_campaign",
        run_mode="ra2021_double_pendulum_coarse_same_window_shard",
        forms=(args.form,),
