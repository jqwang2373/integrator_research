...
- Main proof labels present: `True`.
- Flat proof labels present: `True`.
- Proof object counts match expected: `True`.
...
- Theorem-scope primitive blocker ledger/pc2-open boundary: `True/True`.
- Taylor finite-implication/nonclosure statements main/flat: `True/True`.
...
- Newton-Euler obligation IDs present: `True`.
...
- P7 output nonclaim/residual-to-error boundary in manuscript: `True`.
- B1 closure-scope boundary in main/flat TeX and PDF text: `True`.
- P6 solver-scope boundary in source TeX plus PDF reader text: `True`.
- P1/P2 compact-tube boundary in source TeX plus PDF reader text: `True`.
- P3/P4 implementation-defect boundary in main/flat TeX and PDF text: `True`.
- P5 direct-route boundary in main/flat TeX and PDF text: `True`.
- Proof causality in main/flat TeX and PDF text: `True`.
...
- Auxiliary-evidence scope in main/flat TeX and PDF text: `True`.
...
| `P5` | `d5_direct_substitution_certificate_closed` | `False` | `True` | `True` | D5_DYNAMIC_DIRECT_SUBSTITUTION_CERTIFICATE proves 36 dynamic rows have zero residual on Z_G using closed D1/D2/D3/D4/D6 inputs and no circular P_state/P_acc/P_lambda route |
| `P6` | `finite_probe_recorded_theorem_condition_retained` | `True` | `False` | `False` | accepted_newton_residual_strong_local_inverse_required=true; same reduced-chart initial state and Gauss-predictor branch selection retained; arbitrary Newton initializations and remote nonlinear roots excluded; summary residuals recorded; finite one-step scaled-tolerance probe has 4/4 rows ok; finite short-trajectory scaled-tolerance probe has 4/4 rows and 30 steps ok; uniform all-transition scaled solver-policy theorem still open and not needed to state the conditional theorem; closure would be needed only to promote P6 from retained hypothesis to discharged solver theorem |
| `P7` | `open_residual_to_error_transfer` | `False` | `False` | `False` | 7 residual-to-error obligations remain blocking |
