...
from paper_paths import LATEX, package_path as manuscript_path, LEGACY
ROOT = PAPER.parent
...
def resolve_work_label(rel_path: str) -> Path:
    """Evidence labels in CLAIM_BOUNDARY.json are logical: version dirs live in numerics/, the
    manuscript in paper/, legacy drafts in validation/legacy_drafts/, the rest in validation/."""
    first = rel_path.split("/")[0]
    if re.fullmatch(r"v\d{3}_.+", first):
        return ROOT.parent / "numerics" / rel_path
    if first == "paper":
        return ROOT.parent / rel_path
...
def check_claim_boundary_json(checks: Checks, summary: dict, boundary: dict) -> None:
    ...
    required_evidence_files = {
        ...
        "paper_tex": "legacy_drafts/main.tex",
        "paper_pdf": "legacy_drafts/main.pdf",
    ...
    for key, rel_path in required_evidence_files.items():
        ...
        path = resolve_work_label(rel_path)
...
def main() -> int:
    ...
    print(f"pdf_size_bytes={(LEGACY / 'main.pdf').stat().st_size}")
    return 0
