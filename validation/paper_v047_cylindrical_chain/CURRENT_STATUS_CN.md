...
## 2026-09-20 补充五：仓库重构为五个顶层目录

- 顶层现在是 `paper/`（只放当前稿子及其派生版本）、`proof/`（Lean 拷贝，原 `paper_v047_cylindrical_chain/lean/`）、
  `numerics/`（全部 `vNNN_*`、`reproduction/`、v046 scratch）、`external/`（镜像 + `literature/` 两篇参考 PDF）、
  `validation/`（原 `lie_group_integrator_work/`：账本、验证器、docs、tools、`legacy_drafts/`）。venv 在仓库根
  `.venv_sbel/`，slides 进了 `paper/slides/`。
- `legacy_drafts/` 收纳被取代的 `main.tex`、`main_concise.tex`（含 PDF）和压缩前备份；它们不在 `paper/` 里，
  但账本还在验证它们，所以保留。
