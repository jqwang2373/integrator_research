...
- Moved from `paper_v047_cylindrical_chain/` to `paper/`: `main_cmame.tex`
...
### 2026-09-20 five top-level folders (no claim-state change)

- The repository root is now `paper/` (manuscript only), `proof/` (Lean copy, formerly
  `paper_v047_cylindrical_chain/lean/`), `numerics/` (all `vNNN_*` directories, `reproduction/`,
  the v046 scratch), `external/` (mirrors plus `literature/` with the reference PDFs), and
  `validation/` (this tree, formerly `lie_group_integrator_work/`: the ledger, validators, docs,
  tools, `legacy_drafts/`, `pipeline_validation_results/`, `skills/`). The virtualenv is
  `.venv_sbel/` at the repository root; the talks moved to `paper/slides/`.
- `legacy_drafts/` holds the superseded `main.tex` and `main_concise.tex` with their PDFs and the
  pre-compaction manuscript backups. `paper/` keeps only the current manuscript and its derived
  copies. The legacy drafts are still validated (`validate_paper_claims.py`,
  `validate_concise_paper.py`, the package wrapper); `CLAIM_BOUNDARY.json` points `paper_tex` /
  `paper_pdf` at `legacy_drafts/`.
- `paper_v047_cylindrical_chain/paper_paths.py` defines `REPO`, `VALIDATION`, `LATEX`, `PROOF`,
  `NUMERICS`, `V047`, `V048`, `LITERATURE`, `LEGACY`, `VENV_PYTHON` and `package_path(label)`.
  Stored relative labels were rewritten consistently across scripts, records and docs:
  `../v047_…`/`../v048_…` → `../../numerics/…`, `../.venv_sbel` → `../../.venv_sbel`,
  `../../s11044-026-10153-w.*` → `../../external/literature/…`; `ROOT / "v0NN_…"` constructions
  became `ROOT.parent / "numerics" / …`; the top-level validator enumerates versions in
  `NUMERICS_DIR`. The numerics scripts that referenced the ledger (`run_v048.py`, the forensic
  audit, `reproduction/run_reproduction.py`, `validate_v047_outputs.py`) were repointed.
