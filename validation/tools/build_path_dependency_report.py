...
LATEX = WORK.parent / "paper"  # manuscript (repository root); reported with the prefix "../../paper/"
PROOF = WORK.parent / "proof"  # Lean development copy; prefix "../../proof/"
LEGACY = WORK / "legacy_drafts"  # superseded drafts; prefix "../legacy_drafts/"
...
MANUSCRIPTS = {"../../paper/main_cmame.tex": "CMAME manuscript source (authoritative)",
               "../../paper/cmame_submission_flat/main_cmame_submission.tex": "Elsevier flat copy (derived from main_cmame.tex)",
               "../../paper/arxiv/main_arxiv.tex": "arXiv preprint (derived by build_arxiv_version.py)",
               "../legacy_drafts/main.tex": "legacy internal status report (superseded, kept for the ledger)",
               "../legacy_drafts/main_concise.tex": "legacy concise draft (superseded, kept for the ledger)"}
...
def family_of(rel: str) -> str:
    ...
    if rel.startswith("../../proof/"):
        return "lean"
    if rel.startswith("../legacy_drafts/"):
        return "derived_output" if name.endswith((".pdf", ".log", ".txt")) else "note"
    if rel.startswith("notes/"):
        return "note"
    if rel.startswith("../../paper/arxiv/") or rel.startswith("../../paper/cmame_submission_flat/"):
        return "submission_copy"
    if rel.startswith("../../paper/"):
        return "derived_output" if name.endswith((".pdf", ".log", ".txt")) else "submission_sidecar"
...
def main() -> int:
    ...
    def collect(root: Path, prefix: str) -> list[tuple[Path, str]]:
        return sorted(((p, prefix + str(p.relative_to(root)).replace("\\", "/")) for p in root.rglob("*") if p.is_file()
                       and "__pycache__" not in p.parts and ".lake" not in p.parts and not p.name.startswith(".")
                       and not p.name.endswith(byproducts)), key=lambda pr: pr[1])
    pairs = collect(PAPER, "") + collect(LATEX, "../../paper/") + collect(PROOF, "../../proof/") + collect(LEGACY, "../legacy_drafts/")
    ...
    for s, txt in scripts.items():
        for rel in rels:
            ...
            plain = rel[len("../../paper/"):] if rel.startswith("../../paper/") else rel
            if plain in txt or base in txt or (len(stem) > 12 and stem in txt):
                refs[s].add(rel)
    ...
    payload = {
        ...
        "paper_dir": "validation/paper_v047_cylindrical_chain",
        "manuscript_dir": "paper (reported with the prefix ../../paper/)", "proof_dir": "proof (prefix ../../proof/)", "legacy_dir": "validation/legacy_drafts (prefix ../legacy_drafts/)",
