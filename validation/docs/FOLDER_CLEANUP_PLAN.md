...
## Done On 2026-09-20 (second pass: five top-level folders)

- Repository root now has `paper/`, `proof/`, `numerics/`, `external/`, `validation/` (this tree, the
  former `lie_group_integrator_work/`), plus the untracked `.venv_sbel/`.
- `numerics/` holds all `vNNN_*` directories, `reproduction/` and the v046 scratch; `proof/` is the Lean
  copy (formerly `paper_v047_cylindrical_chain/lean/`); `external/literature/` holds the reference PDFs;
  `paper/slides/` holds the talks; `validation/legacy_drafts/` holds the superseded `main.tex` and
  `main_concise.tex` with their PDFs and the manuscript backups.
- Scripts reach the other folders through `paper_v047_cylindrical_chain/paper_paths.py` (`LATEX`,
  `PROOF`, `NUMERICS`, `LITERATURE`, `LEGACY`, `package_path`). Stored relative labels were rewritten
  consistently (`../v048_…` → `../../numerics/v048_…`, `../.venv_sbel` → `../../.venv_sbel`,
  `../../s11044…` → `../../external/literature/s11044…`).
...
1. Remove Python cache directories after a validation pass:
   ...
   - `validation/v*/__pycache__/`
   - `validation/paper_v047_cylindrical_chain/__pycache__/`
