...
A copy of the Lean sources (no build artefacts) lives in the repository at `proof/`;
`build_exact_stage_identity_gate.py` records whether that copy is byte-identical to the development below.
...
output and also checks that the repository copy `proof/` is byte-identical to the development.
