...
_VENV_PYTHON = ROOT.parent / ".venv_sbel" / "bin" / "python"
...
_LEGACY_TOPLEVEL = {"main.tex", "main.pdf", "main.log", "main.txt", "main_concise.tex", "main_concise.pdf", "main_concise.log", "main_concise.txt"}


def paper_asset_path(label: str) -> Path:
    first = str(label).replace("\\", "/").split("/")[0]
    if first in _MANUSCRIPT_TOPLEVEL or first in _MANUSCRIPT_DIRS:
        return LATEX_DIR / label
    if first in _LEGACY_TOPLEVEL:
        return LEGACY_DIR / label
    return PAPER_DIR / label


RESULTS = ROOT / "pipeline_validation_results"
CURRENT_VERSION = "v047"
CURRENT_DIR = NUMERICS_DIR / "v047_cylindrical_chain_pipeline"
V048_DIR = NUMERICS_DIR / "v048_cross_paper_same_test_benchmarks"
...
def rel_label(path: Path) -> str:
    """Logical label of a path: version-directory files relative to numerics/ (as before the
    2026-09-20 restructure), everything else relative to validation/ or the repository root."""
    path = Path(path)
    for base in (NUMERICS_DIR, ROOT, REPO):
        try:
    ...
    for path in files:
        ...
        if suffix == ".json":
            ...
            try:
                ...
                v.check(False, f"JSON parse failed for {rel_label(path)}: {exc}")
            else:
                v.check(True, f"JSON parse ok for {rel_label(path)}")
        elif suffix == ".csv":
            counts["csv"] += 1
            try:
                rows = read_csv_rows(path)
            except Exception as exc:  # noqa: BLE001
                v.check(False, f"CSV parse failed for {rel_label(path)}: {exc}")
            else:
                v.check(len(rows) > 0, f"CSV has no data rows: {rel_label(path)}")
        elif suffix == ".png":
            counts["png"] += 1
            try:
                width, height = png_size(path)
            except Exception as exc:  # noqa: BLE001
                v.check(False, f"PNG parse failed for {rel_label(path)}: {exc}")
            else:
                v.check(width > 0 and height > 0, f"PNG has invalid size: {rel_label(path)}")
        elif suffix == ".md":
            counts["md"] += 1
            v.check(path.stat().st_size > 0, f"empty markdown artifact: {rel_label(path)}")
        else:
            counts["other"] += 1
            v.check(path.stat().st_size > 0, f"empty artifact: {rel_label(path)}")
    return counts
...
def validate_version_inventory(v: Validator) -> tuple[list[dict[str, object]], dict[str, int]]:
    ...
    for version_dir in version_dirs:
        ...
        report = NUMERICS_DIR / str(ledger.get("report", ""))  # ledger stores version-relative labels
        ...
        v.check(report.exists(), f"{version} report missing: {rel_label(report) if report != ROOT else report}")
        ...
        inventory.append(
            {
                ...
                "summary": str(rel_label(summary)) if summary else "",
                "report": str(rel_label(report)) if report.exists() else str(ledger.get("report", "")),
...
def run_paper_claim_validator(v: Validator) -> tuple[str, ...]:
    required_files = [
        LEGACY_DIR / "main.tex",
        LEGACY_DIR / "main.pdf",
        LEGACY_DIR / "main_concise.tex",
        LEGACY_DIR / "main_concise.pdf",
    ...
    for path in required_files:
        v.check(path.exists() and path.stat().st_size > 0, f"paper package file missing or empty: {rel_label(path)}")
    ...
    for token in [
        ...
        "`../CURRENT_PIPELINE_CONTRACT.md`",
        "`../../numerics/v047_cylindrical_chain_pipeline/results/summary_v047.json`",
        "`validate_cmame_submission.py`",
        "`validate_submission_artifact_manifest_boundary_sync.py`",
        "`validate_cmame_blocker_closure_gate.py`",
        "`validate_cmame_external_baseline_gate.py`",
...
def validate_no_claim_regression_wording(v: Validator) -> None:
    ...
    rel_paths = [
        ...
        "docs/ORDER_PROOF_LEDGER.md",
        "docs/VALIDATION_QUICKSTART.md",
        "../numerics/v047_cylindrical_chain_pipeline/README.md",
        "../numerics/v047_cylindrical_chain_pipeline/results/v047_report.md",
        "paper_v047_cylindrical_chain/README.md",
