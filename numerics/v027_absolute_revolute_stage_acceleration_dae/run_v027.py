...
def run_experiment() -> dict:
    ...
    write_csv(RESULTS / "absolute_revolute_stage_acceleration_runs.csv", rows)
...
def plot_results(summary: dict) -> None:
    ...
    plt.title("Stage Velocity/Acceleration Absolute Revolute DAE Convergence")
    plt.grid(True, which="both", alpha=0.35)
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "absolute_revolute_stage_acceleration_convergence.png", dpi=180)
    plt.close()
    ...
    plt.legend(fontsize=7)
    plt.tight_layout()
    plt.savefig(RESULTS / "absolute_revolute_stage_acceleration_constraints.png", dpi=180)
    plt.close()
...
def write_report(summary: dict) -> None:
    ...
    (RESULTS / "v027_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    summary = {
        "version": "v027_absolute_revolute_stage_acceleration_dae",
        ...
        "model": {
            ...
            "reference_h": REF_H,
            "t_final": T_FINAL,
            "constraints": "pivot position plus hinge-axis alignment; PivotVA adds stage pivot velocity and acceleration consistency",
            "endpoint_projection": "optional revolute SHAKE/RATTLE-style projection only for projected baseline",
            "new_feature": "square stage pivot velocity plus pivot acceleration constraint residual",
        },
        "experiment": experiment,
        "runtime_sec": time.perf_counter() - started,
    }
    with (RESULTS / "summary_v027.json").open("w", encoding="utf-8") as f:
