...
CASES = {"brown_mcphee_smooth": 0.5, "brown_mcphee_sharp": 0.05}
TOLERANCES = [1.0e-4, 1.0e-5, 1.0e-6, 3.0e-7, 1.0e-7]
FIXED_RUNS = [
    ("gauss4_fixed_h005", "lambda_gauss4_endpoint_jax", 0.05),
    ("gauss4_fixed_h0025", "lambda_gauss4_endpoint_jax", 0.025),
    ("gauss4_fixed_h00125", "lambda_gauss4_endpoint_jax", 0.0125),
    ("gauss6_fixed_h005", "lambda_gauss6_endpoint_jax", 0.05),
...
def brown_mcphee_lambda_friction_torque(w: np.ndarray, lam: np.ndarray, params) -> np.ndarray:
    w = np.asarray(w, dtype=float).reshape(3)
    lam = np.asarray(lam, dtype=float).reshape(3)
    normal_load = float(np.linalg.norm(lam))
    vs = max(float(params.friction_eps), 1.0e-12)
    z = w / vs
    denom = (0.25 * z * z + 0.75) ** 2
    mu_curve = params.mu_d * np.tanh(4.0 * z) + (params.mu_s - params.mu_d) * z / denom
    scale = params.friction_radius * normal_load / np.sqrt(3.0)
    viscous = params.viscous_damping * np.tanh(4.0) * w
    return -scale * mu_curve - viscous


def _brown_mcphee_lambda_friction_torque_jax(
    w, lam, mu_s, mu_d, stribeck_velocity, friction_eps, viscous_damping, friction_radius
):
    _ = stribeck_velocity
    normal_load = v019.jnp.linalg.norm(lam)
    vs = v019.jnp.maximum(friction_eps, 1.0e-12)
    z = w / vs
    denom = (0.25 * z * z + 0.75) ** 2
    mu_curve = mu_d * v019.jnp.tanh(4.0 * z) + (mu_s - mu_d) * z / denom
    scale = friction_radius * normal_load / v019.jnp.sqrt(v019.jnp.array(3.0, dtype=w.dtype))
    viscous = viscous_damping * v019.jnp.tanh(v019.jnp.array(4.0, dtype=w.dtype)) * w
    return -scale * mu_curve - viscous


def install_brown_mcphee_model() -> None:
    v019.lambda_friction_torque = brown_mcphee_lambda_friction_torque
    v019._lambda_friction_torque_jax = _brown_mcphee_lambda_friction_torque_jax


install_brown_mcphee_model()
...
def adaptive_run(tol: float, params, ref_state, case_name: str) -> tuple[dict, dict]:
    ...
    row = {
        "case": case_name,
        "run_type": "adaptive",
        "label": f"adaptive_bm_lambda_gauss64_tol{tol:.0e}",
        "method": "adaptive_brown_mcphee_lambda_gauss64_endpoint_jax",
        "tolerance": f"{tol:.1e}",
        "h": "",
...
def run_experiment() -> dict:
    rows = []
    cases = {}
    for case_name, stribeck_velocity in CASES.items():
        params = v019.make_params(friction_eps=stribeck_velocity)
        ...
        fixed_orders = {}
        for method in ["lambda_gauss4_endpoint_jax", "lambda_gauss6_endpoint_jax"]:
            method_runs = [item for item in fixed_summaries if item["method"] == method]
            method_runs = sorted(method_runs, key=lambda item: item["h"], reverse=True)
            fixed_orders[method] = {
                "orientation_observed_order": qp.estimate_order(
                    [item["h"] for item in method_runs], [item["orientation_error_rad"] for item in method_runs]
                ),
                "omega_observed_order": qp.estimate_order(
                    [item["h"] for item in method_runs], [item["omega_l2_error"] for item in method_runs]
                ),
            }
        fastest_better = [
            item
        ...
        cases[case_name] = {
            "brown_mcphee_stribeck_velocity": stribeck_velocity,
    ...
    write_csv(RESULTS / "brown_mcphee_lambda_runs.csv", rows)
...
def plot_results(summary: dict) -> None:
    ...
    for case_name, case in summary["cases"].items():
        for group_name in ["fixed", "adaptive"]:
            ...
            for x, y, label in zip(xs, ys, labels):
                short = label.replace("adaptive_bm_lambda_gauss64_", "adap_").replace("gauss6_fixed_", "g6_")
    ...
    plt.title("Brown-McPhee Lambda-Friction: Adaptive Gauss64 vs Fixed Steps")
    plt.grid(True, which="both", alpha=0.35)
    plt.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(RESULTS / "brown_mcphee_lambda_error_runtime.png", dpi=180)
    plt.close()
...
def write_report(summary: dict) -> None:
    ...
    for case_name, case in summary["cases"].items():
        ...
        g4_order = case["fixed_orders"]["lambda_gauss4_endpoint_jax"]
        g6_order = case["fixed_orders"]["lambda_gauss6_endpoint_jax"]
        ...
        lines.append(
            f"- `{case_name}` fixed-step observed orientation order: Gauss4 "
            f"{g4_order['orientation_observed_order']:.3f}, Gauss6 {g6_order['orientation_observed_order']:.3f}."
        )
        lines.append(
        ...
        elif error_ratio_vs_fine < 1.0 and value_vs_fine > 1.0:
            lines.append(
                f"- `{case_name}` conclusion: adaptive is a high-accuracy accuracy-per-cost win; it "
                f"beats globally halving Gauss6 by {1.0 / error_ratio_vs_fine:.1f}x in error at "
                f"{runtime_ratio_vs_fine:.2f}x runtime, but it is not the low-cost default."
            )
        else:
            lines.append(
    ...
    (RESULTS / "v021_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> None:
    ...
    summary = {
        "version": "v021_brown_mcphee_lambda_friction",
        "python": platform.python_version(),
        "platform": platform.platform(),
        "numpy": np.__version__,
        "model": {
            "cases": CASES,
            "friction_curve": "Brown-McPhee componentwise continuous velocity curve with lambda-scaled normal load",
    ...
    with (RESULTS / "summary_v021.json").open("w", encoding="utf-8") as f:
