...
def gauss_legendre_coefficients(n_stages: int) -> tuple[Array, Array, Array]:
    if n_stages == 2:
        root3 = np.sqrt(3.0)
    ...
    if n_stages == 3:
        ...
        return c, A, b
    raise ValueError(f"unsupported Gauss stage count: {n_stages}")


def _gauss_legendre_coefficients_jax(n_stages: int, dtype) -> tuple[object, object, object]:
    if n_stages == 2:
        root3 = jnp.sqrt(jnp.array(3.0, dtype=dtype))
        c = jnp.array([0.5, 0.5], dtype=dtype) + jnp.array([-root3 / 6.0, root3 / 6.0], dtype=dtype)
        A = jnp.array(
            [
                [0.25, 0.25 - root3 / 6.0],
                [0.25 + root3 / 6.0, 0.25],
...
def _stage_guess(state: State, h: float, params: Params, n_stages: int = 2) -> Array:
    cs, _, _ = gauss_legendre_coefficients(n_stages)
...
def _unpack_stages(x: Array, n_stages: int = 2) -> list[dict[str, Array]]:
    stages = []
    offset = 0
...
def _endpoint_unpack(x: Array, n_stages: int = 2) -> tuple[list[dict[str, Array]], dict[str, Array]]:
    offset = 21 * n_stages
    stages = _unpack_stages(x[:offset], n_stages)
    end = {
        "u": x[offset : offset + 3],
...
def _unpack_stages_jax(x, n_stages: int = 2):
    stages = []
    offset = 0
    for _ in range(n_stages):
        stages.append(
...
def _endpoint_residual_jax_core_n(
    x,
    r0,
    p0,
    v0,
    ...
    _, A, b = _gauss_legendre_coefficients_jax(n_stages, x.dtype)
    ...
    for i, st in enumerate(stages):
        ...
        u_coll = st["u"] - h * sum(A[i, j] * ks[j] for j in range(n_stages))
        w_coll = st["w"] - w0 - h * sum(A[i, j] * stages[j]["alpha"] for j in range(n_stages))
...
def _endpoint_residual2_jax_core(x, *args):
    return _endpoint_residual_jax_core_n(x, *args, n_stages=2)


def _endpoint_residual3_jax_core(x, *args):
    return _endpoint_residual_jax_core_n(x, *args, n_stages=3)


_endpoint_residual2_value = jax.jit(_endpoint_residual2_jax_core)
_endpoint_residual2_jacobian = jax.jit(jax.jacfwd(_endpoint_residual2_jax_core, argnums=0))
_endpoint_residual3_value = jax.jit(_endpoint_residual3_jax_core)
_endpoint_residual3_jacobian = jax.jit(jax.jacfwd(_endpoint_residual3_jax_core, argnums=0))
...
def absolute_gauss_endpoint_step(
    ...
    stage0 = _stage_guess(state, h, params, n_stages=n_stages)
    stages0 = _unpack_stages(stage0, n_stages=n_stages)
    ...
    if n_stages == 2:
        residual_value = _endpoint_residual2_value
        residual_jacobian = _endpoint_residual2_jacobian
    elif n_stages == 3:
        residual_value = _endpoint_residual3_value
        residual_jacobian = _endpoint_residual3_jacobian
    else:
        raise ValueError(f"unsupported Gauss stage count: {n_stages}")
    ...
    for it in range(max_iters):
        ...
        if last_norm < tol:
            break
        J = np.asarray(residual_jacobian(x_jax, *args), dtype=float)
    ...
    stages, end = _endpoint_unpack(x, n_stages=n_stages)
...
def integrate(method: str, h: float, t_final: float, params: Params | None = None) -> dict:
    ...
    for _ in range(n_steps):
        if method == "reduced_rkmk4":
            ...
            state, niters, diag = absolute_gauss_endpoint_step(state, h, params, n_stages=2)
            total_iters += niters
        ...
        elif method == "quaternion_gauss_lie6_endpoint_jax":
            state, niters, diag = absolute_gauss_endpoint_step(state, h, params, n_stages=3, max_iters=18)
            total_iters += niters
            max_stage_constraint = max(max_stage_constraint, diag["max_stage_constraint_norm"])
            max_stage_force = max(max_stage_force, diag["max_stage_force_residual_norm"])
            max_stage_torque = max(max_stage_torque, diag["max_stage_torque_residual_norm"])
