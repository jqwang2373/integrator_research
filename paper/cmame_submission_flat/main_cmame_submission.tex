...
\begin{enumerate}
  ...
  chain and on the four public benchmark mechanisms, every number generated from result files by
  scripts in the accompanying repository.
\end{enumerate}
...
All experiments use the implementation of Section~\ref{sec:method} with fixed Newton tolerances
($\tau_N=10^{-11}$ for the chain and the pendulums, $10^{-10}$ for the closed-loop runner) and the
dense automatic-differentiation Jacobian; the scripts that produce every
...
\paragraph{Diagnostics of the analysis}
The constants and hypotheses of Section~\ref{sec:order} are measured on the same runs and
reported in \ref{app:diagnostics}: the converged stage vectors satisfy the exact stage identity to
roundoff (Table~\ref{tab:exact-identity-check}); the inverse bound of Lemma~\ref{lem:p2-from-p1}
holds with ratio at most $1.55$ although its sufficient Neumann condition would need
$h\le8\times10^{-4}$ (Table~\ref{tab:p2-constants-check}); the implemented predictor sits at
distance about $50$ from the Gauss stage (Table~\ref{tab:predictor-check}); and under the fixed
tolerance the accepted residual is $10^{-14}$ to $8\times10^{-12}$, so that (H3) holds with
$c_\eta$ growing from $10^{-4}$ to $10^{6}$ across the sweep (Table~\ref{tab:e7}).
...
dense factorization; in the present Python implementation the wall time per step of the three-stage member is only
$15$ to $25\%$ above that of the one-stage member, both being dominated by interpreter overhead. At $h=0.0125$ the three-stage
...
reactions. Each mechanism has its own assembly of the stage system of Section~\ref{sec:method} (the same
row families with the joint list of \ref{app:lower-pair}); \method{} is run on all four over
$T=1$ with $h=0.1/2^k$ (Figure~\ref{fig:e4}, Table~\ref{tab:e4}). For the driven
single pendulum the harness starts its stage solve from the analytic drive, so this panel checks
the endpoint reconstruction and the roundoff level rather than the convergence of the stage solve:
the final-state position and orientation errors are $5.9\times10^{-10}$ and $3.9\times10^{-10}$ at
$h=0.1$, $9.3\times10^{-12}$ and $6.0\times10^{-12}$ at $h=0.05$, $1.5\times10^{-13}$ and
$9.4\times10^{-14}$ at $h=0.025$ (pairwise orders $6.0$ and $6.0$, the quadrature error of the
Gauss-weight reconstruction) and at roundoff, $10^{-14}$ to $10^{-16}$, for every finer step. Its
velocity errors are not reported: the stage Jacobian of this driven problem has condition number
$10^{10}$ to $10^{13}$, and with a predictor already at the solution the accepted velocities carry
roundoff amplified by that conditioning ($10^{-11}$ to $10^{-8}$, with no trend in $h$). For the two closed loops the trajectory errors against the
...
\begin{table}[!htb]
\centering
\caption{The four benchmark mechanisms over $T=1$: final-state errors (single pendulum: position and
orientation against the analytic driven motion; double pendulum: position and velocity against the
$h=0.1/128$ reference)
...
\toprule
$h$ & \multicolumn{3}{l}{driven single pendulum (analytic reference)} & \multicolumn{3}{l}{double pendulum (free)} \\
    & position & orientation & order & position & velocity & order \\
\midrule
$0.1$ & $5.94\mathrm{e}{-10}$ & $3.85\mathrm{e}{-10}$ & 6.0 & $4.59\mathrm{e}{-5}$  & $1.56\mathrm{e}{-4}$  & 8.2 \\
$0.05$ & $9.32\mathrm{e}{-12}$ & $5.97\mathrm{e}{-12}$ & 6.0 & $1.58\mathrm{e}{-7}$  & $1.85\mathrm{e}{-7}$  & 7.0 \\
$0.025$ & $1.45\mathrm{e}{-13}$ & $9.36\mathrm{e}{-14}$ & -- & $1.24\mathrm{e}{-9}$  & $1.48\mathrm{e}{-9}$  & 6.0 \\
$0.0125$ & $1.08\mathrm{e}{-14}$ & $5.95\mathrm{e}{-15}$ & -- & $1.93\mathrm{e}{-11}$ & $2.29\mathrm{e}{-11}$ & 6.0 \\
$0.00625$ & $1.58\mathrm{e}{-14}$ & $9.40\mathrm{e}{-15}$ & -- & $3.10\mathrm{e}{-13}$ & $3.86\mathrm{e}{-13}$ & 3.8 \\
$0.003125$ & $1.33\mathrm{e}{-15}$ & $1.55\mathrm{e}{-16}$ & -- & $2.29\mathrm{e}{-14}$ & $6.15\mathrm{e}{-14}$ & -- \\
\midrule
...
{\method{} on the four benchmark mechanisms over $T=1$. The driven single pendulum shows the
reconstruction error (order six) down to roundoff, the two driven loops are at roundoff for every
step size, and the double pendulum shows the sixth order of the method.}
{fig:e4}
